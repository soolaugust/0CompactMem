...
[![Tests](https://img.shields.io/badge/tests-3543%20passing-brightgreen)](#测试)
[![License](https://img.shields.io/badge/license-MIT-green)](LICENSE)
[![Iterations](https://img.shields.io/badge/iterations-1051%2B-orange)](#路线图)
...
openMnemos 将**操作系统内存管理哲学**应用于 AI 认知资源管理。让 Linux 用有限 RAM 处理数百万进程的同一套原理，现在赋予 AI Agent 持久化、可检索、多 Agent 共享的记忆。
...
| Pin / mlock 语义 | ✅                       | ❌             | ❌             | ❌             |
| 测试覆盖         | 3,500+ tests             | 各异           | 各异           | 各异           |

> **一句话总结** — 如果你想要一个能 `pip install`、在笔记本上以 sidecar 运行、被多个 Claude Code / Cursor / 自定义 agent 共享、并且能用操作系统心智模型推理的记忆层，openMnemos 就是为此而生的。如果你需要托管云服务或完整 agent 运行时，请考虑上面的替代方案。
...
| 冲突检测 + chunk 碎片合并 | ext4 fsck + 块合并 | iter 371–374 |
| 情绪显著性检索加分 | DRD4 多巴胺奖励信号 | iter 376 |
| Schema 扩散激活 | Bartlett 图式理论 | iter 380 |
| 间隔效应调度器 | Ebbinghaus 遗忘曲线 | iter 383 |
| 返回抑制（IOR） | Posner（1980）注意力转移 | iter 391 |
| 上下文相似度加分 | 依赖上下文的编码 | iter 394 |
| 舌尖现象（TOT）恢复 | FTS5 零命中边缘激活 | iter 425 |
| 序列位置效应排序 | Murdock（1962）首因/近因 | iter 427 |
| 二次机会多样性采样 | Clock 页面置换算法 | iter 471 |
| Token 预算感知摘要截断 | 内存压力分层 | iter 474 |
| 加载器页表去重（vm_flags） | MMU 页表遍历 | iter 526 |
| CFS 按 chunk 带宽限流 | CFS 带宽控制器 | iter 560 |
| 渐进带宽惩罚 | TCP 拥塞窗口 | iter 600–612 |
| 时间爆发抑制（24h/7d） | Token 桶速率限制 | iter 614–618 |
| WAL 免疫注入时间线 | Journal WAL 屏障 | iter 647–648 |
| 时间线幽灵条目 GC | rmap 反向映射回收 | iter 659–660 |
| 最终抑制门禁 | OOM kill 最终调节 | iter 663 |
| 跨项目召回统计 | cgroups memcg 跨命名空间统计 | iter 566 |
| 长查询 generic 分类器旁路 | TLB 缺失快路径 | iter 710 |
| 中英双语信号扩充 | iconv 字符集归一化 | iter 722 |
| 新 session 首注入保护 | exec() 地址空间重置 | iter 804 |
| 短时爆发抑制（6h 窗口） | Swap token 公平性 | iter 813 |
| 多样性计数器轮询 | CFS 组调度 | iter 872 |
| 小库多样性加分 | NUMA 本地分配 | iter 898 |
| 全局跨项目 suppress | cgroup v2 统一层级 | iter 1024 |
| 项目集中度惩罚 | NUMA 拓扑距离 | iter 1029 |
| 跨类型总量硬限 | memcg 硬限制 | iter 1050 |
...
| 认知检索模型 — 情绪显著性、Schema 激活、IOR、TOT、间隔效应（iter 375–482） | ✅ 完成 |
| 检索精度 — 带宽限流、爆发抑制、WAL 免疫时间线、幽灵 GC（iter 526–700） | ✅ 完成 |
| 多层多样性抑制 — 跨项目统计、集中度惩罚、类型硬限（iter 700–1051） | ✅ 完成 |
| 分布式多 Agent 共享内存 — NUMA/RDMA 类比（iter 1052+） | 🔜 计划中 |
