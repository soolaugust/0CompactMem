...
\begin{table}[t]
\centering
\caption{Retrieval quality (20 paraphrased queries, 20 items). Measured via \texttt{benchmark\_comparison.py}.}
\label{tab:retention}
\small
\begin{tabular}{lcc}
\toprule
...
Vector (embedding)     & \textbf{0.90} & \textbf{0.73} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Analysis.} Embedding-based vector search substantially outperforms BM25 on paraphrased queries (Recall@5: 0.90 vs.\ 0.35). This is expected and unsurprising---semantic similarity handles paraphrases (``auth system design'' $\to$ ``JWT tokens'') that lexical matching cannot. We present this honestly: \textbf{0CompactMem does not claim retrieval superiority}. Its value proposition lies elsewhere (see Benchmarks 2--3).
...
\paragraph{Analysis.} Under 4$\times$ pressure (keeping 50 of 250 chunks), the vector search baseline retains 0\% of constraints---it has no mechanism to distinguish ``must keep'' from ``can evict.'' 0CompactMem's hard-pinned constraints are excluded from eviction by construction; the measured 82\% reflects retrieval verification (not all pinned items match the verification query due to BM25 limitations), but all 50 pinned entries physically remain in the database. Systems without explicit pin semantics \emph{cannot} provide any survival guarantee regardless of implementation sophistication.
...
\begin{table}[!htb]
\centering
\caption{Search latency comparison (measured, 100 items).}
\label{tab:latency}
\small
\begin{tabular}{lcc}
\toprule
...
Vector search (embedding)   & 35.0           & 42.1 \\
\bottomrule
\end{tabular}
\end{table}
